...
return new class extends Migration
{
    ...
    {
        Schema::create('allies_management', function (Blueprint $table) {
            $table->id();
            $table->text('wolkvox_id');
            $table->foreignId('user_id')->constrained('users')->onDelete('cascade');
            $table->foreignId('contact_id')->constrained('allies_contacts')->onDelete('cascade');
            $table->foreignId('payroll_management_id')->constrained('payrolls')->onDelete('cascade')->nullable();
            $table->boolean('solution')->default(1);
            $table->foreignId('consultation_id')->constrained('allies_consultations')->onDelete('cascade');
            $table->foreignId('specific_id')->nullable()->constrained('allies_specifics')->nullOnDelete();
    ...
    {
        Schema::dropIfExists('allies_management');
    }
};